    ...
    request('POST', '/v1/e/broadcast', tk, {
      ...
      setTimeout(() => {
        request('GET', `/v1/e/accumulate?id=move&start=${now}`, tk, null, (accResponse) => {
          ...
          console.log(`Twitch made ${accResponse.data.length} clicks over ${Object.keys(uservote).length} unique users.`);
...
function win(gb) {
  for (let j = 0; j < 3; j += 1) {
    ...
    if (sum === PLAYER * 3) { return PLAYER; }
    if (sum === TWITCH * 3) { return TWITCH; }
